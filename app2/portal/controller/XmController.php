...
//ajax 接收处理数据
    public function ajaxtype(){
        ...
        foreach($info as $k=>$v){
            ...
            $html.="<a href='".$url."/'>";
            $html.='<div class="img">';
            $html.="<img src='/themes/simpleboot3/public/mobile/xin/images/f3d98677b149cee3971cf6331de6d8f6.jpg' class='lazy'  data-original='".'http://www.91chuangye.com/upload/'.$v['mobile_thumbnail']."' alt='".$v['name']."'>";
            $html.='</div>';
    ...
    public function listajax(){
        ...
        $data = Db::name('portal_xm')
            ->alias('por')
            ->field('por.aid,por.class,por.litpic,por.thumbnail,por.title,por.sum,por.invested,por.companyname,por.address,por.nativeplace,cat.name as categoryname')
            ->join('portal_category cat','por.typeid = cat.id')
            ->where($where)->where('por.status = 1 and por.arcrank = 1')->order('update_time desc')->limit($page,10)
            ->select();
        $dataa = $data->all();

        foreach ($dataa as $key => $value) {
            if(isset($value['nativeplace']) && ($value['nativeplace']!='')){
                $nativeplace = db('sys_enum')->where("evalue = ".$value['nativeplace']." and py != ''")->value("ename");
                $dataa[$key]['address'] = $nativeplace;
            }
        }
        $html='';
        foreach ($dataa as $k=>$v){
            ...
            $html.='<a href="javascript:;">'.str_replace('加盟','',$v['categoryname']).'</a>';
            $html.='<a href="javascript:;">'.$v['address'].'</a>';
            $html.='</div>';
