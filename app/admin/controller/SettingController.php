...
//    }
    ...
    public function clearCache()
    {
        ...
        $redis->clear();
        //删除目录
        $data = Db::name('html')->select();
        foreach($data as $data){
            //删除index
            if(strpos($data['path'],'index.html')){
                if(file_exists(CMF_ROOT.$data['path'])){
                    @unlink(CMF_ROOT.$data['path']);
                    Db::name('html')->delete($data['id']);
                }
            }else{
                if(file_exists(CMF_ROOT.$data['path'])){
                    @unlink(CMF_ROOT.$data['path'].'/index.html');
					@rmdir(CMF_ROOT.$data['path'].'/');
                    Db::name('html')->delete($data['id']);
                }
            }
        }
        //特殊情况删除top
        @rmdir(CMF_ROOT.'/public/top/');
        @rmdir(CMF_ROOT.'/m/top/');
        @rmdir(CMF_ROOT.'/public/news/');
        @rmdir(CMF_ROOT.'/m/news/');
