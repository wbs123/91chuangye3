    ...
    public function comdoLogin()
    {
        if ($this->request->isPost()) {
            ...
            if(cmf_check_mobile($data['username'])) {
                $user['mobile'] = $data['username'];
                $log            = $userModel->doMobile($user,3);
            } else {
				$user['user_login'] = $data['username'];
                $log                = $userModel->doName($user,3);
            }
